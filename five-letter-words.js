...
//     db.run(`DROP TABLE IF EXISTS words`, [], (err, row) => console.log('drop', err));
//     db.run(`CREATE TABLE words (id INTEGER PRIMARY KEY AUTOINCREMENT, word VARCHAR(5))`, [], (err, row) => { console.log('create', err); throw err });
...
function addWord(word) {
    const cmd = `INSERT INTO words (word) VALUES ('${word}')`;
    db.run(cmd);
}
